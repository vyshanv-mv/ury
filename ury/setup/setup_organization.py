...
def check_setup_status():
    return {
        ...
        "has_company": bool(frappe.db.get_all("Company", limit=1))
    }

@frappe.whitelist(allow_guest=True)
def get_organization_context():
    # Return some defaults if company doesn't exist
    return {
        "company_name": "",
        "abbr": "",
        "country": "India",
        "currency": "INR",
        "user_name": frappe.session.user if frappe.session.user != "Guest" else "",
        "email": frappe.session.user if frappe.session.user != "Guest" else "",
    }
