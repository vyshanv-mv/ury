...
export interface Table {
  ...
  no_of_seats?: number;
  custom_layout_x?: number;
  custom_layout_y?: number;
...
export async function getTables(room: string): Promise<Table[]> {
  ...
  const [apiRes, dbRes] = await Promise.all([
    ...
    db.getDocList(DOCTYPES.URY_TABLE, {
      fields: ['name', 'custom_layout_x', 'custom_layout_y', 'minimum_seating', 'table_shape', 'no_of_seats'],
  ...
  return apiTables.map(t => {
    ...
    return {
      ...
      custom_layout_x: dbT?.custom_layout_x,
      custom_layout_y: dbT?.custom_layout_y,
...
export async function createTable(data: Partial<Table>) {
